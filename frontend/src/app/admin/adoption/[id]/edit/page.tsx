...
export default function EditAdoptionListingPage() {
  ...
  useEffect(() => {
    const fetchListing = async () => {
      try {
        const result = await adoptionListingApi.getListingById(listingId);
        if (result.data) {
          const l = result.data;
          setFormData({
            name: l.name || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await adoptionListingApi.updateListing(
        listingId,
      ...
      router.push(`/admin/adoption/${listingId}`);
    } catch {
  ...
  return (
    <AdminLayout>
      <div className="space-y-4">
        <button
          onClick={() => router.push(`/admin/adoption/${listingId}`)}
          ...
          <div>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              ...
              {existingPhotos.map((url, i) => (
                <div
                  key={`existing-${i}`}
                  className="relative w-20 h-20 rounded-xl overflow-hidden shrink-0 border-2 border-teal-200"
                >
                  <Image
                    src={url}
                    alt={`Photo ${i + 1}`}
              ...
              {newPhotoPreviews.map((preview, i) => (
                <div
                  key={`new-${i}`}
                  className="relative w-20 h-20 rounded-xl overflow-hidden shrink-0 border-2 border-blue-200"
                >
                  <Image
                    src={preview}
                    alt={`New ${i + 1}`}
            ...
            <p className="text-xs text-gray-400 mt-1">
              New photos will replace existing ones if uploaded.
            </p>
          </div>
